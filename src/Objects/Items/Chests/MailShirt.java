...
public class MailShirt extends Item {
    int HP = 40;
    int DMG = 0;
    int crit = 5;
    int magic = 10;
...
/*   public DrakeHelmet(int HP, int DMG, int crit, int magic) {
    ...
    public void eqON(Player player) {
        if (chestEquip < 2) {
            player.setMaxHP(player.getMaxHP() + MailShirt.this.HP);
            player.setHP(player.getHP() + MailShirt.this.HP);
            player.setDMG(player.getDMG() + MailShirt.this.DMG);
            player.setCritChance(player.getCritChance() + MailShirt.this.crit);
