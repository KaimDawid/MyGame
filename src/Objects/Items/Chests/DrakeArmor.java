...
public class DrakeArmor extends Item {
    int HP = 35;
    int DMG = 10;
    int crit = 5;
    int magic = 25;
    ...
    String name = ("Smocza zbroja [CHEST] (" +HP +"HP, " + DMG + "DMG, +" + crit +"% szans na crit, " + magic + " siły zaklęć)");
    int helmEquip;
    int chestEquip = 1;
    int handsEquip;
    int weaponEquip;
...
/*   public DrakeHelmet(int HP, int DMG, int crit, int magic) {
    ...
    boolean daEquip = false;

    @Override
    public void eqON(Player player) {
        if (chestEquip < 2) {
            player.setMaxHP(player.getMaxHP() + DrakeArmor.this.HP);
            player.setHP(player.getHP() + DrakeArmor.this.HP);
            player.setDMG(player.getDMG() + DrakeArmor.this.DMG);
            player.setCritChance(player.getCritChance() + DrakeArmor.this.crit);
            player.setMagic(player.getMagic() + DrakeArmor.this.magic);
            chestEquip = 2;
