...
public class LeatherGloves extends Item {
    ...
    String name = ("Skórzane rękawice [HANDS] (" +HP +"HP, " + DMG + "DMG, +" + crit +"% szans na crit, " + magic + " siły zaklęć)");
    int helmEquip;
    int chestEquip;
    int isON = 0;
    int handsEquip = 1;
    ...
    int eqValue;
    public LeatherGloves(int stock) {
        this.stock = stock;
    }
...
/*   public DrakeHelmet(int HP, int DMG, int crit, int magic) {
    ...
    boolean lgEquip = false;

    @Override
    public void eqON(Player player) {
        if (handsEquip < 2) {
            player.setMaxHP(player.getMaxHP() + LeatherGloves.this.HP);
            player.setHP(player.getHP() + LeatherGloves.this.HP);
            player.setDMG(player.getDMG() + LeatherGloves.this.DMG);
            player.setCritChance(player.getCritChance() + LeatherGloves.this.crit);
            player.setMagic(player.getMagic() + LeatherGloves.this.magic);
            handsEquip = 2;
            System.out.println("Założyłeś " + name);
            eqValue = 1;

        }
        else if (lgEquip && handsEquip > 1){
            player.setMaxHP(player.getMaxHP() - LeatherGloves.this.HP);
