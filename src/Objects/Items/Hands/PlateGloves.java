...
public class PlateGloves extends Item {
    ...
    String name = ("Płytowe rękawice [HANDS] (" +HP +"HP, " + DMG + "DMG, +" + crit +"% szans na crit, " + magic + " siły zaklęć)");
    int helmEquip;
    int chestEquip;
    int isON = 0;
    int handsEquip = 1;
...
/*   public DrakeHelmet(int HP, int DMG, int crit, int magic) {
    ...
    boolean pgEquip = false;

    @Override
    public void eqON(Player player) {
        if (handsEquip < 2) {
            player.setMaxHP(player.getMaxHP() + PlateGloves.this.HP);
            player.setHP(player.getHP() + PlateGloves.this.HP);
            player.setDMG(player.getDMG() + PlateGloves.this.DMG);
            player.setCritChance(player.getCritChance() + PlateGloves.this.crit);
            player.setMagic(player.getMagic() + PlateGloves.this.magic);
            handsEquip = 2;
            System.out.println("Założyłeś " + name);
            eqValue = 1;

        }
        else if (pgEquip && handsEquip > 1){
            player.setMaxHP(player.getMaxHP() - PlateGloves.this.HP);
