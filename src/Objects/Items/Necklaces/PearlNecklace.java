...
public class PearlNecklace extends Item {
    ...
    int eqValue;
    public PearlNecklace(int stock) {
        this.stock = stock;
    }
...
/*   public DrakeHelmet(int HP, int DMG, int crit, int magic) {
    ...
    public void eqON(Player player) {
        if (neckEquip < 2) {
            player.setMaxHP(player.getMaxHP() + PearlNecklace.this.HP);
            player.setHP(player.getHP() + PearlNecklace.this.HP);
            player.setDMG(player.getDMG() + PearlNecklace.this.DMG);
            player.setCritChance(player.getCritChance() + PearlNecklace.this.crit);
